...
$(function(){
	...
	var deviceNameStr = '';  //群发的设备名称
	//显示给那个情报板发布信息
	var getSendId = localStorage.getItem('cmsIdForPlan').split(',');
	deviceList(getSendId);
	function deviceList(getSendId){
		$.ajax({
		    type:"get",
		    url:"/deviceList",   
	  		data:{
...
//} 
	...
	var cmsSizeArr = localStorage.getItem('cmsSizeForPlan').split(',');
	...
	function previewAdd(){
       ...
       // $(".editPart .preview").html(text);		
		...
		for(var i = 0; i < allCmsVal.length; i++) {	
			...
			var cmsAttr = {'deviceCode':jsonObjItem,'deviceName':deviceNameStr,'deviceType':boardSizeStr,'commandType':$('.commandType').val(),'stayTime':$('.time').val(),'contents':previewConAngXy,'font':fontFamilyIndex,'fontSize':switchFontSizeVal(fontSize),'fontFrontColor':fontColorIndex,'align':alignIndex,'content':text,'groupDescribe':$('.groupDescribe').val()};
...
//		}
	...
	function savePulish(){
		...
		localStorage.setItem('savePulishJson',JSON.stringify(savePulishJson));
		var box = $("#box");
    	 box.addClass("open");
         box.find('.faIcon').removeClass('fa-times').addClass('fa-check');
         box.find('.fontCon').html('请点击！');
