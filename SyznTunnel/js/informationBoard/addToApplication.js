...
function defaultFont(){
	//黑体
	$('.defaultFontFamily').siblings().removeClass('blueBorder');
	$('.defaultFontFamily').addClass('blueBorder');
	$('.fontFamilyItemHidden').text($('.defaultFontFamily').text());
	$('.fontFamilyItemIndexHidden').text($('.defaultFontFamily').attr('index'));
	
	//黄色
	var thisColor = $('.defaultFontColor').attr('colorAttr');
	$('.defaultFontColor').parent().siblings().css('border-color','');
	$('.defaultFontColor').parent().siblings().children().removeClass('borderSelect').addClass('borderUnSelect');
	$('.defaultFontColor').parent().css('border-color',thisColor);
	$('.defaultFontColor').removeClass('borderUnSelect').addClass('borderSelect',thisColor);
	$('.fontColorHidden').text($('.defaultFontColor').attr('colorAttr'));
	$('.fontColorIndexHidden').text($('.defaultFontColor').attr('index'));
}
//选择颜色
...
function selectItem(obj){
	...
	var divNnm = 1;
	function previewAdd(){
		console.log('previewAdd===============================================================previewAdd');
       ...
       // $(".editPart .preview").html(text);		
		...
		for(var i = 0; i < allCmsVal.length; i++) {	
			...
			if(jsonObjItem.length > 1){
				deviceCode = jsonObjItem;
			}else{
				deviceCode = unique(jsonObjItem);
			}
			var cmsAttr = {'deviceCode':deviceCode,'deviceType':boardSizeStr,'commandType':$('.commandType').val(),'stayTime':$('.time').val(),'contents':previewConAngXy,'font':fontFamilyIndex,'fontSize':switchFontSizeVal(fontSize),'fontFrontColor':fontColorIndex,'align':alignIndex,'content':text,'groupDescribe':$('.groupDescribe').val()};
//			var cmsAttr = {'cmsCode':i+"cms",'deviceCodes':jsonObjItem,"message":[{'deviceType':boardSize,'commandType':$('.commandType').val(),'time':$('.time').val(),'contents':previewConAngXy,'font':fontFamilyIndex,'fontSize':switchFontSizeVal(fontSize),'fontFrontColor':fontColorIndex,'align':alignIndex,'content':text}]};
            ...
            //html +='               <span class="fa fa-trash-o" cmsAttr="'+cmsAttr+'"></span>';
           html +="               <span class='fa fa-trash-o' cmsAttr='"+cmsAttr+"'></span>";
            html +='         </div>';
            html +='    </div>';
            html +='</div>';
