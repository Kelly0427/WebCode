...
function modalFun(){
	$('.devicecode').click(function(){
		...
		if(conObj.linkagedevice){
			$('#showVideo').attr('linkagedevice',conObj.linkagedevice).show();
		}else{
			$('#showVideo').removeAttr('linkagedevice').hide();
		}
	})
}
function getCms(){
	...
	function informationBoardPublish(message){
		...
		$.each(messsgeStr,function(index,item){
			...
			if(item.deviceCode == nowDeviceCode){
				...
				Explain(variantValue,variantNO,deviceType);
			}
		})
		
	}
}
function Explain(variantValue,variantNO,deviceType){
...
function showVideo(btn){
	initVideoActive();
	findByDeviceCode($(btn).attr('linkagedevice'))
